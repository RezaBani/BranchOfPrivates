...
LetterDialog::LetterDialog(const QString &tab, const QString &code, const QMap<QString, QStringList> &combosMap, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::LetterDialog)
{
    ...
    if (!dir.exists()){
        showLetterWarning("پوشه پرونده سربازان یافت نشد");
        return;
    }
    ...
    if(!fileExist){
        showLetterWarning("فایل پرونده سرباز یافت نشد");
        return;
    }
    ...
    for(int i = 0; i < letters.size(); i++){
        ...
        for(const QString &code: codes){
            if(code == "A"){
                ...
                number->setMinimum(1);
                number->setMaximum(Soldier::maxSpinBox());
...
void LetterDialog::btnAddClicked()
{
    ...
    QStringList values;
    for(QWidget *widget: stackedWidget->currentWidget()->findChildren<QWidget*>(QString(), Qt::FindDirectChildrenOnly)){
    ...
    SoldierFiles *soldierFiles = new SoldierFiles(m_combosMap,this);
    ...
    connect(soldierFiles,&SoldierFiles::addToOrder,this,&LetterDialog::sendAddToOrder);
    connect(soldierFiles,&SoldierFiles::changeInOrder,this,&LetterDialog::sendChangeInOrder);
    connect(soldierFiles,&SoldierFiles::removeOrder,this,&LetterDialog::sendRemoveOrder);
...
void LetterDialog::sendChangeInOrder(const QString &sheetName, const QStringList &values, const int &newValues)
{
    emit changeInOrder(sheetName, values, newValues);
}

void LetterDialog::sendRemoveOrder(const QString &sheetName, const QStringList &values)
{
    emit removeOrder(sheetName, values);
}
